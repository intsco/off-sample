from collections import Counter
import numpy as np
from matplotlib import pyplot as plt


def imshow(ax, inp, stats, title=None):
    """ Imshow for Tensor """
    inp = inp.transpose((1, 2, 0))
    inp = stats['std'] * inp + stats['mean']
    inp = np.clip(inp, 0, 1)
    ax.imshow(inp)
    if title is not None:
        plt.title(title)


def plot_inputs(rows, cols, input_iter, stats):
    inputs, classes = [t.numpy() for t in next(input_iter)]
    most_common_classes = [ci for ci, _ in Counter(classes).most_common(3)]

    for cl_idx in most_common_classes:
        f, axes = plt.subplots(nrows=rows, ncols=cols, figsize=(3 * cols, 3 * rows))
        inputs_ = inputs[classes == cl_idx]

        if len(inputs_.shape) > 4:
            bs, ncrops, c, h, w = inputs_.shape
            inputs_ = inputs_.reshape(-1, c, h, w)

        for i in range(min(len(inputs_), rows * cols)):
            imshow(axes[i % rows, i % cols], inputs_[i], stats)
        f.suptitle(f'class {cl_idx}', y=0.91);
        plt.show()


